...
{
  HELIB_TIMER_START;
  //std::cout<<"PubKey::Encrypt*********//////"<<std::endl;
  ...
  // generate a random encryption of zero from the public encryption key 从公共加密密钥中产生零的随机加密
  ctxt = pubEncrKey; // already an encryption of zero, just not a random one 已经是零的加密，只是不是随机的
                     // ctxt with two parts, each with all the ctxtPrimes  ctxt有两部分，每个部分都有所有的ctxtPrimes
  ctxt.noiseBound = 0;
  ...
  for (size_t i = 0; i < ctxt.parts.size(); i++) { // add noise to all the parts
    ...
    //std::cout<<"ctxt.parts[i]:"<<ctxt.parts[i]<<std::endl;
                          ...
                          long maxDegKswitch)
{
  ...
  //std::cout << "skBounds: " << bound << std::endl;
                            ...
                            long p)
{
  ...
  DoubleCRT s_ = sKeys.at(toIdx); // this can be a reference 这可以作为参考
  ...
  if (palg.getPow2() == 0) // not power of two
    stdev *= sqrt(palg.getM());
  std::vector<DoubleCRT> e1;
  std::vector<DoubleCRT> e2;
  // size-n vector
  ksMatrix.b.resize(
      n,
  ...
  ksMatrix.a.resize(
      n,
      DoubleCRT(context, context.ctxtPrimes | context.specialPrimes));
  e1.resize(n, DoubleCRT(context, context.ctxtPrimes | context.specialPrimes));
  e2.resize(n, DoubleCRT(context, context.ctxtPrimes | context.specialPrimes));
...
**/
  ...
  double bound2 = 0;
  {
    ...
    for (long i = 0; i < n; i++) {
      ksMatrix.a[i].randomize();
      ksMatrix.b[i] = ksMatrix.a[i];
      ksMatrix.b[i] += 1;
      ksMatrix.a[i].Negate();
      bound1 = e1[i].sampleGaussianBounded(stdev);
      e1[i] *= p;
	  //Reject sample
      bound2 = e2[i].sampleGaussianBounded(stdev * stdev * 1.2);
      e2[i] *= p;
    }
    bound1 *= skBounds[fromIdx] * p;
    bound2 *= p;
    bound2 += bound1;
    ksMatrix.noiseBound = bound2 ;
  ...
  s_ *= context.productOfPrimes(context.specialPrimes);
  NTL::ZZ productDi = context.productOfPrimes(context.digits[0]);
  ksMatrix.b[0] *= fromKey;
  ksMatrix.a[0] *= s_;
  ksMatrix.b[0] += e2[0];
  ksMatrix.a[0] += e1[0];
  for (long i = 1; i < n; i++) {
    ksMatrix.b[i] *= fromKey;
    ksMatrix.b[i] *= productDi;
    ksMatrix.a[i] *= s_;
    ksMatrix.a[i] *= productDi;
    productDi *= context.productOfPrimes(context.digits[i]);
...
#if 0
  // HERE
  std::cout << "*** ksMatrix: " << fromSPower << " " << fromXPower << " "
            << fromIdx << " " << toIdx << " " << p << " "
            << "\n";
                     ...
                     NTL::ZZX& f) const // plaintext before modular reduction
{
  ...
  //std::cout << "plaintxt mod p:"  << plaintxt << std::endl;
